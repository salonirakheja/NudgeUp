...
interface Commitment {
  ...
  name: string;
  icon: string;
  streak?: number; // Optional - if provided, use it; otherwise calculate
  completed: boolean;
}
  ...
  return (
    <div className="w-full flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3">
        {habits.map((habit) => {
          // Use streak from habit object if provided, otherwise calculate from context
          const streak = habit.streak !== undefined ? habit.streak : getCommitmentStreak(habit.id);
